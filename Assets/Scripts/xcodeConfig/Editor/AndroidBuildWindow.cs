...
namespace MHEditor.Build 
{
    ...
    public class AndroidBuildWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Build App Bundle"))
            {
        ...
        private static void BuildAPK()
        {
            ...
            if (!string.IsNullOrEmpty(androidData.apkPath))
            {
                ...
                Debug.Log($"apkname=={apkName}");
            }
            string path = EditorUtility.SaveFilePanel("Build Android", parentDirectory, apkName, "apk");
            if(!string.IsNullOrEmpty(path))
            {
                androidData.apkPath = path;
        ...
        private static void BuildAppBundle()
        {
            ...
            string parentDirectory = System.Environment.CurrentDirectory;
            string aabName = "";
            if (!string.IsNullOrEmpty(androidData.aabPath))
            {
                FileInfo aabFileInfo = new FileInfo(androidData.aabPath);
                parentDirectory = aabFileInfo.DirectoryName;
                aabName = aabFileInfo.Name;
            }
            string path = EditorUtility.SaveFilePanel("Build Android", parentDirectory, aabName, "aab");
            if (!string.IsNullOrEmpty(path))
            {
                androidData.aabPath = path;
                if (EditorUserBuildSettings.activeBuildTarget != BuildTarget.Android)
                    EditorUserBuildSettings.SwitchActiveBuildTarget(BuildTargetGroup.Android, BuildTarget.Android);
                BuildPipeline.BuildPlayer(GetBuildScenes(), androidData.aabPath, BuildTarget.Android, BuildOptions.None);
                SaveData();
                EditorUtility.RevealInFinder(androidData.aabPath);
                DeleteFiles();
            }
        }
