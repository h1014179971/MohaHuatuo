...
using MHEditor.Build;

public class XCodeConfig : MonoBehaviour {

    static string configPath = "Assets/xcodeConfig/Editor/XCodeConfig.json";
    ...
    public static void OnPostprocessBuild(BuildTarget buildTarget, string path) {
        if (buildTarget == BuildTarget.iOS) {
			configPath = IosBuildWindow.ConfigPath;
        ...
        if (table!=null) {
            ...
            if (addList != null) {
                foreach (string i in addList) {
                    string framework = i;
                    if(OptionalFarmework(framework))
