...
namespace UIFramework
{
    public class UIController : MonoBehaviour
    {
        ...
        private UIPage _CurrentTopPage;
        public UIPage CurrentTopPage
        {
            get
            {
                return _CurrentTopPage;
            }
        }
        ...
        private void Awake()
        {
            ...
            _uiTopRoot = transform.Find("UITopRoot");
        ...
        public T OpenTopPage<T>(string path, object args = null) where T : UIPage
        {
            if (_PageTypeList.Contains(typeof(T)))
            {
                // Exists page, Show it, hide current page
                if (typeof(T) == _CurrentTopPage.GetType())
                {
                    // Trying to show current page  
                    Debug.LogWarning("Trying to show current window.");
                    return _CurrentTopPage as T;
                }
                else
                {
                    // Accessing hided page, close current page and show hided page
                    ...
                    _PageTypeList.Remove(typeof(T));
                    _PageTypeList.Add(typeof(T));
                    _CurrentTopPage.Hide();
                    _CurrentTopPage = _PageDict[typeof(T)];
                    _CurrentTopPage.Open();
                    return _CurrentTopPage as T;
                }
            }
            else
            {
                ...
                if (pageObj)
                {
                    return OpenTopPage<T>(pageObj, args);
                }
                else
                {
                    Debug.LogError("Prefab NO FOUND! Path:" + path);
                    return null;
                }
            }
        }

        public T OpenTopPage<T>(GameObject pageObj, object args = null) where T : UIPage
        {
            if (_PageTypeList.Contains(typeof(T)))
            {
                // Exists page, Show it, hide current page
            ...
            else
            {
                ...
                // Generate new page
                _CurrentTopPage = GeneratePanel<T>(pageObj, _uiTopRoot);
                _PageTypeList.Add(typeof(T));
                _PageDict.Add(typeof(T), _CurrentTopPage);
