...
namespace ColaFramework
{
    public class AudioCtrl : IManager
    {
        ...
        private class AudioSourcePlayer : IDisposable
        {
            ...
            {
                audioSource.Play();
                timerId = Timer.Register(audioSource.clip.length, () => { HandlePlayEnd(); });
            }
        ...
        {
            AudioClip clip = ExtractIdleAudioClip(name);
            for (int i = 0; i < _multipleAudio.Count; i++)
            {
        ...
        public void StopAllWorldSound()
        {
            foreach (KeyValuePair<GameObject, AudioSource> audio in _worldAudio)
            {
                if (audio.Value!=null && audio.Value.isPlaying)
                {
                    audio.Value.Stop();
                }
            }
        ...
        public void SwitchAudioListener(bool isOn)
        {
            if (isOn)
